...
function parseLevel(index) {
    ...
    if (!data) {
        console.error(`Level index ${index} is out of range`);
        return false;
    }
...
function initGame() {
    ...
    if (!canvas) {
        console.error('Unable to find the game canvas element.');
        return;
    }

    ctx = canvas.getContext('2d');
    if (!ctx) {
        console.error('Unable to obtain a 2D rendering context.');
        return;
    }
